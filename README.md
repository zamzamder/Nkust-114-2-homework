...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Project1.Web` and `Project1.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 66fec0fa-ecd1-4332-b595-226cb9b4c7b1
```

> `66fec0fa-ecd1-4332-b595-226cb9b4c7b1` is the password of the certificate, you can change it to any password you want.
...
Run `Project1.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Project1.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Project1.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
